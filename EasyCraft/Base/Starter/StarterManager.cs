...
namespace EasyCraft.Base.Starter
{
    public class StarterManager
    {
        ...
        {
            foreach (string file in Directory.EnumerateFiles(Directory.GetCurrentDirectory() + "/data/starters",
                "*.dll"))
            {
                Type type = null;
                ...
                {
                    type = Assembly.Load(File.ReadAllBytes(file), File.ReadAllBytes(Path.ChangeExtension(file, "pdb")))
