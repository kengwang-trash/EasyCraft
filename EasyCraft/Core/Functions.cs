...
namespace EasyCraft.Core
{
    internal class Functions
    {
        ...
        public static string GetRandomString(int length = 15, bool useNum = true, bool useLow = true,
        ...
        {
            // 1 - 创建用户组
            if (UserGroup.IsGroupExist("easycraft")) FastConsole.PrintWarning(Language.t("没有找到用户组,正在创建"));
        }
 ...
 GetMD5HashFromFile(System.Diagnostics.Process.GetCurrentProcess().MainModule.FileName + ".bak");
        ...
        public static void CopyDirectory(string sourceDirPath, string SaveDirPath)
        {
            ...
            //递归，遍历文件夹
            foreach (var dir in dirs) CopyDirectory(dir, SaveDirPath + "/" + Path.GetFileName(dir));
        }
