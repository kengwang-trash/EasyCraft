...
{
    internal class WebPanelPhraser
    {
        ...
        public void PhraseWeb(HttpListenerRequest req, HttpListenerResponse res)
        {
            ...
            if (session.ContainsKey("auth")) vars.user = new User(session["auth"]);

            if (uri.AbsolutePath.StartsWith("/api/"))
            {
                var absolutepage = uri.AbsolutePath.Substring(5);
            ...
            {
                var absolutepage = uri.AbsolutePath.Substring(8);
                if (CheckAssetsPath(absolutepage))
                {
                    ...
                    else if (absolutepage.EndsWith("css")) response.ContentType = "text/css";
        ...
        public void Print404()
        {
            ...
            PrintWeb("<h1>404 Not Found</h1><hr />Path: " + uri.AbsoluteUri + "<br />Time:" + DateTime.Now +
        ...
        public void PrintWeb(string echo)
        {
            try
            {
                var buff = Encoding.UTF8.GetBytes(echo);
        ...
        private static Dictionary<string, string> PhrasePOST(string postraw)
        {
            ...
            var ps = postraw.Split('&');
            foreach (var pd in ps)
            {
                var key = Uri.UnescapeDataString(pd.Substring(0, pd.IndexOf("=")));
                var value = Uri.UnescapeDataString(pd.Substring(pd.IndexOf("=") + 1));
                ans[key] = value;
            }
